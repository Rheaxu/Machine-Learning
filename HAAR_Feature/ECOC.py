...
"""
-------------------------------------------------------------------------------
About: ECOC (Error Correcting Output Codes) For HAAR features
        (Slightly modified from the original one for data format)
Author: Ruiyu Xu
Time: Nov. 2014
-------------------------------------------------------------------------------
"""
...
#		invert_y_list[int(label)].append(i)
		for i,ftr in enumerate(X):
			train_ftr_vals_list[i].append(ftr)
...
def generate_stumps():
	...
	for i,vals_list in enumerate(train_ftr_vals_list):
		sort_vals_list = sorted(vals_list)
		if len(sort_vals_list) == 0:
			ftrs_threshold_list.append([-0.05,0.0,0.05])
			continue
		non0_list = [0.0]
		non0_list.extend(sort_vals_list)
		max_threshold = sort_vals_list[-1]+0.05
		threshold_list = [-0.05]
	...
	for i,stumps in enumerate(ftrs_threshold_list):
		...
		for j,stump in enumerate(stumps):
			...
			if j == 0:
				...
				cur_start = 0
				for i,val in enumerate(ftr_vals):
					if val > stump:
						if y_list[i] == 1.0:
							TPlist.append(i)
						else:
...
def generate_funcs():
	...
	all_func_no = len(all_func_list)
	bin_no = all_func_no/(func_no+2)
	for i in range(2,func_no+2):
		func_list.append(all_func_list[all_func_no-i*bin_no+	\
					(i+1)*int(M.pow(-1,i/2))])
	...
	for i in range(0,k):
		...
		for f in func_list:
			codeword.append(f[i])
		if codeword in code_list:
			print "yes"
		code_list.append(codeword)

def run():
	global X_list,orig_y_list,y_list,invert_y_list,test_X_list,	\
			test_y_list,ds_size,test_size
	generate_funcs()

	#ds_size = 500
	#X_list,orig_y_list,orig_y_invert_list = get_train_var_list(trainlines)
	all_X_list,all_orig_y_list,test_X_list,test_y_list = HAAR.sample_data()
	ds_size = len(all_orig_y_list)
	X_list,orig_y_list = get_some_train_data(ds_size,all_X_list,all_orig_y_list)
	get_train_ftr_var_list()
	#get_test_var_list
...
def get_some_train_data(size,all_X,all_y):
	all_data = []
	print "The length of all_X is",len(all_X)
	print "The length of all_y is",len(all_y)
	for i,X in enumerate(all_X):
		data = X+[all_y[i]]
		all_data.append(data)
	random.shuffle(all_data)
	print "After shuffle, the len of all_data is",len(all_data)
	some_data = all_data[:size]
	print "The len of some_data is",len(some_data)
	some_X,some_y = [[],[]]
	for d in some_data:
		some_X.append(d[:-1])
		some_y.append(d[-1])
	"Finished to get",size,"data"
